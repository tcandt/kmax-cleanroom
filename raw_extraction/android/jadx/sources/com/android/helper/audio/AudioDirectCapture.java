...
public class AudioDirectCapture implements AudioCapture {
    ...
    private static AudioRecord createAudioRecord(int i) {
        AudioRecord.Builder builderM = FakeContext$$ExternalSyntheticApiModelOutline0.m();
        if (Build.VERSION.SDK_INT >= 31) {
            builderM.setContext(FakeContext.get());
        }
        builderM.setAudioSource(i);
        builderM.setAudioFormat(AudioConfig.createAudioFormat());
        int minBufferSize = AudioRecord.getMinBufferSize(48000, 12, 2);
        if (minBufferSize > 0) {
            builderM.setBufferSizeInBytes(minBufferSize * 8);
        }
