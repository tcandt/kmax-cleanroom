...
B4_DIMENSIONS_SPEC = [
    ...
    },
    {
        ...
        "go_test_target": {
            "pkg": "./tests",
            "test_name": "TestCameraBackpressureSCTPE2E",
        },
    },
    {
        "id": "CAM-B4-DIM-16",
...
def evaluate_b4_dimension(dim: Dict[str, Any], test_cache: Dict[str, Any], repo_root: Path) -> Tuple[str, Dict[str, Any]]:
    ...
    if tgt:
        ...
        if res.get("exit_code") != 0 or not res.get("package_passed"):
            return "FAILED", {
                "error": f"Test {tname} failed in {pkg}",
...
def run_b4_verifier_mutation_tests(
    ...
    failing_cache_6 = {
        "./tests::TestCameraBackpressureSCTPE2E": {
            "exit_code": 1,
            "package_passed": False,
            "tests": {"TestCameraBackpressureSCTPE2E": {"action": "fail"}},
        }
    }
...
def main():
    ...
    if args.check:
        ...
        fresh_counters = result.get("counters", {})

        if existing_meta.get("status") != fresh_meta.get("status") or existing_counters != fresh_counters:
            print("[FAIL] Check failed: freshly derived differential differs from existing file!", file=sys.stderr)
            print(f"  Existing counters: {existing_counters}", file=sys.stderr)
            print(f"  Fresh counters:    {fresh_counters}", file=sys.stderr)
            sys.exit(1)
        print(f"[+] B4 Differential check PASSED (13/13 negative mutations verified, {fresh_meta.get('total_original_parity_requirements')} parity requirements, 0 failures)")
        sys.exit(0)
    ...
    print(f"    - Negative Mutations:    13/13 PASSED")
