#!/usr/bin/env python3
"""
reproduce_license_forensics.py - Phase 2C.3HR2 License Forensic Reproducibility Tool

Verifies that ALL canonical Phase 2C.3H/HR License & Entitlement forensic artifacts
are 100% reproducible directly from canonical ELF, ROUTE_HANDLER_MAP, FUNCTION_MAP,
and dynamic oracle, adhering to cleanroom invariants with zero hardcoded authority.

Strict Invariants:
  - Zero authoritative literals as PASS criteria: all names/VAs are derived by machine traversal.
    Historical observations may only be logged as PREVIOUS_OBSERVATION.
  - Denominator discovered dynamically from canonical artifact manifest: LICENSE_FORENSIC_REPRODUCIBILITY = X/X.
...
# Canonical Manifest covering all Phase 2C.3H and Phase 2C.3HR License artifacts
...
def verify_license_reproducibility():
    ...
    print("==========================================================")
    print(f"PHASE 2C.3HR2 LICENSE FORENSIC REPRODUCIBILITY ({total_manifest}/{total_manifest})")
    print("==========================================================")
    ...
    # 2. Activation Request Struct Descriptor Derivation from activation handler
                ...
                if op.type == capstone.x86.X86_OP_MEM and op.mem.base == capstone.x86.X86_REG_RIP:
                    ...
                    if tgt_off and 0 <= tgt_off < len(elf_bytes) - 64:
                        if (elf_bytes[tgt_off+23] & 0x1f) == 25: # KindStruct
                            ...
                            if st_sz == 16: # string size in Go AMD64
                                # Parse field to confirm json:"license"
    ...
    # 3. Status-Builder Helper Derivation from common callees
    ...
    for c in (stat_callees & dbg_callees):
        if c.startswith("main.") and c in fm_by_sym:
            ...
            has_makemap = any("makemap" in x for x in c_callees)
            has_assign = any("mapassign" in x for x in c_callees)
            if has_makemap and has_assign:
                status_builder_candidates.append(c)

    if len(status_builder_candidates) != 1:
        print(f"[FAIL] Expected 1 unique status-builder helper, got {status_builder_candidates}")
        return False
    ...
    key_sha256 = hashlib.sha256(pub_key_bytes).hexdigest()
    print(f"[PASS] 6. PUBLIC_KEY_VERIFIER: XOR=0x{xor_key:02x}, Key={pub_key_bytes.hex()[:16]}..., SHA256={key_sha256[:16]}...")
    ...
    # =========================================================================
    # B. DEEP SEMANTIC CONTENT VALIDATION ACROSS ALL ARTIFACTS
    # =========================================================================
    ...
    for a_name in CANONICAL_ARTIFACT_MANIFEST:
        ...
        if a_name == "LICENSE_ROUTE_METHOD_MATRIX.json":
            # Semantic check: each route/verb status, content-type, and CORS
            for r_pat in target_routes:
                if r_pat not in rj:
                    print(f"[FAIL] Missing route {r_pat} in method matrix")
                    return False
                for verb in ["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"]:
                    cv = cj[r_pat][verb]
                    rv = rj[r_pat][verb]
                    if cv["status_code"] != rv["status_code"]:
                        print(f"[FAIL] Method matrix status mismatch {r_pat} {verb}: {cv['status_code']} != {rv['status_code']}")
                        return False
                    if cv["content_type"] != rv["content_type"]:
                        print(f"[FAIL] Method matrix content-type mismatch {r_pat} {verb}: {cv['content_type']} != {rv['content_type']}")
                        return False
                    if cv["cors_methods"] != rv["cors_methods"]:
                        print(f"[FAIL] Method matrix cors_methods mismatch {r_pat} {verb}: {cv['cors_methods']} != {rv['cors_methods']}")
                        return False

        elif a_name == "LICENSE_AUTH_MATRIX.json":
            # Semantic check: every auth context status code
            for r_pat in target_routes:
            ...
            # Semantic check: all 6 startup cases
            ...
            for sc_c, sc_r in zip(cj["cases"], rj["cases"]):
                for field in ["name", "license_status_http_code", "license_source", "status", "activated", "error_msg"]:
            ...
            if cj["pipeline"][3]["options_equivalence"]["status"] != "PURE_ED25519_EQUIVALENT":
                print("[FAIL] Crypto verification contract options status is not PURE_ED25519_EQUIVALENT")
                return False
            if len(cj["pipeline"]) != 6:
                print(f"[FAIL] Expected 6 pipeline steps, got {len(cj['pipeline'])}")
                return False
            ...
            # Full semantic equivalence for all other contracts
            if cj != rj:
