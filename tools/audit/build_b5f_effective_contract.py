#!/usr/bin/env python3
"""
tools/audit/build_b5f_effective_contract.py
Phase 2C.5B5FR Effective Contract View Builder.

Constructs the effective B5F implementation contract view by deterministically combining:
1. Frozen Base Contract: evidence/go_agent/webrtc/AI_COMMAND_B5F_IMPLEMENTATION_CONTRACT.json
2. Formal Errata Layer:  evidence/go_agent/webrtc/AI_COMMAND_B5F_CONTRACT_ERRATA.json

The base contract is never modified and remains frozen (SHA256: 64642e153dcefaa2857fdc37f16b7dc076915c56aa1b3777519eb0c0d6dbdf65).
The effective contract view cleanly separates authentic original protocol parity from
safety scope guards, defensive validation choices, deferred boundaries, and reference interoperability.
"""

import argparse
import hashlib
import json
import sys
from pathlib import Path
from typing import Any, Dict, List, Optional

ROOT = Path(__file__).resolve().parent.parent.parent

FROZEN_BASE_SHA256 = "64642e153dcefaa2857fdc37f16b7dc076915c56aa1b3777519eb0c0d6dbdf65"
BASE_CONTRACT_REL = "evidence/go_agent/webrtc/AI_COMMAND_B5F_IMPLEMENTATION_CONTRACT.json"
ERRATA_REL = "evidence/go_agent/webrtc/AI_COMMAND_B5F_CONTRACT_ERRATA.json"


def build_b5f_effective_contract(repo_root: Path = ROOT) -> Dict[str, Any]:
    base_path = repo_root / BASE_CONTRACT_REL
    errata_path = repo_root / ERRATA_REL

    if not base_path.exists():
        raise FileNotFoundError(f"B5F Base contract missing: {base_path}")
    if not errata_path.exists():
        raise FileNotFoundError(f"B5F Errata file missing: {errata_path}")

    base_bytes = base_path.read_bytes()
    actual_base_sha = hashlib.sha256(base_bytes).hexdigest().lower()
    if actual_base_sha != FROZEN_BASE_SHA256.lower():
        raise ValueError(
            f"B5F Base contract SHA256 mismatch! Expected frozen {FROZEN_BASE_SHA256}, got {actual_base_sha}"
        )

    base_data = json.loads(base_bytes.decode("utf-8"))
    errata_data = json.loads(errata_path.read_text(encoding="utf-8"))

    errata_meta = errata_data.get("metadata", {})
    ref_sha = errata_meta.get("base_contract_sha256")
    if ref_sha != FROZEN_BASE_SHA256:
        raise ValueError(
            f"B5F Errata references incorrect base contract SHA256: {ref_sha} != {FROZEN_BASE_SHA256}"
        )

    corrections_map = {c["contract_id"]: c for c in errata_data.get("corrections", [])}

    effective_reqs: List[Dict[str, Any]] = []
    by_classification: Dict[str, List[str]] = {
        "original_static_evidence": [],
        "cross_component_evidence": [],
        "reference_interoperability": [],
        "safe_scope_guard": [],
        "defensive_validation": [],
        "deferred_execution_boundary": [],
        "unknown": []
    }

    for req in base_data.get("requirements", []):
        cid = req["id"]
        corr = corrections_map.get(cid)

        eff_req: Dict[str, Any] = {
            "id": cid,
            "base_contract_id": cid,
            "requirement": req.get("requirement", ""),
            "requirement_name": req.get("requirement", ""),
            "source_artifact": req.get("source_artifact", ""),
            "base_observable_behavior": req.get("observable_behavior", ""),
            "base_evidence_class": req.get("evidence_class", ""),
            "base_mandatory_for_parity": req.get("mandatory_for_parity", False),
        }

        if corr:
            eff_req["has_errata_correction"] = True
            eff_req["effective_original_parity_claim"] = corr["corrected_original_parity_claim"]
            eff_req["effective_classification"] = corr["effective_evidence_class"]
            eff_req["effective_mandatory_for_original_parity"] = corr.get("effective_mandatory_for_original_parity", False)
            eff_req["mandatory_for_safe_scope"] = corr.get("mandatory_for_safe_scope", False)
            eff_req["mandatory_for_interoperability"] = corr.get("mandatory_for_interoperability", False)
            eff_req["non_original_compatibility_behavior"] = corr.get("non_original_compatibility_behavior", "")
            eff_req["reference_only_parts"] = corr.get("reference_only_parts", [])
            eff_req["phase_scope_parts"] = corr.get("phase_scope_parts", [])
            eff_req["errata_rationale"] = corr.get("rationale", "")
        else:
            eff_req["has_errata_correction"] = False
            eff_req["effective_original_parity_claim"] = req.get("observable_behavior", "")
            eff_req["effective_classification"] = req.get("evidence_class", "")
            eff_req["effective_mandatory_for_original_parity"] = req.get("mandatory_for_parity", False)
            eff_req["mandatory_for_safe_scope"] = False
            eff_req["mandatory_for_interoperability"] = False
            eff_req["non_original_compatibility_behavior"] = ""
            eff_req["reference_only_parts"] = []
            eff_req["phase_scope_parts"] = []

        eff_req["mandatory_for_parity"] = eff_req["effective_mandatory_for_original_parity"]
        eff_req["evidence_class"] = eff_req["effective_classification"]
        eff_req["observable_behavior"] = eff_req["effective_original_parity_claim"]

        # Classification into distinct taxonomy families
        e_cls = eff_req["effective_classification"]
        if cid == "AI-B5F-06":
            by_classification["defensive_validation"].append(cid)
        elif cid == "AI-B5F-09":
            by_classification["deferred_execution_boundary"].append(cid)
        elif cid in ("AI-B5F-04", "AI-B5F-14") or e_cls in ("REFERENCE_INTEROPERABILITY", "REFERENCE_ONLY"):
            by_classification["reference_interoperability"].append(cid)
        elif cid in ("AI-B5F-10", "AI-B5F-15") or e_cls == "SAFE_SCOPE_GUARD":
            by_classification["safe_scope_guard"].append(cid)
        elif e_cls == "STATIC_CONFIRMED":
            by_classification["original_static_evidence"].append(cid)
        elif e_cls == "CROSS_COMPONENT_CONFIRMED":
            by_classification["cross_component_evidence"].append(cid)
        else:
            by_classification["unknown"].append(cid)

        effective_reqs.append(eff_req)

    return {
        "metadata": {
            "title": "Phase 2C.5B5F Effective Implementation Contract View",
            "phase": "Phase 2C.5B5FR",
            "base_contract_sha256": FROZEN_BASE_SHA256,
            "total_requirements": len(effective_reqs),
            "corrected_requirements_count": len(corrections_map),
            "taxonomy_counts": {
                "original_static_evidence": len(by_classification["original_static_evidence"]),
                "cross_component_evidence": len(by_classification["cross_component_evidence"]),
                "reference_interoperability": len(by_classification["reference_interoperability"]),
                "safe_scope_guard": len(by_classification["safe_scope_guard"]),
                "defensive_validation": len(by_classification["defensive_validation"]),
                "deferred_execution_boundary": len(by_classification["deferred_execution_boundary"]),
                "unknown": len(by_classification["unknown"])
            }
        },
        "breakdown_by_classification": by_classification,
        "requirements": effective_reqs,
        "effective_requirements": effective_reqs,
    }


def main():
    parser = argparse.ArgumentParser(description="Build Phase 2C.5B5F Effective Implementation Contract")
    parser.add_argument("--output", type=str, help="Target path to output effective contract JSON")
    args = parser.parse_args()

    try:
        eff = build_b5f_effective_contract(ROOT)
        b = eff["breakdown_by_classification"]
        tc = eff["metadata"]["taxonomy_counts"]
        print(f"[+] Successfully built effective B5F contract: {eff['metadata']['total_requirements']} requirements ({eff['metadata']['corrected_requirements_count']} corrected via formal errata)")
        print(f"    - Original Static Evidence:       {tc['original_static_evidence']} ({', '.join(b['original_static_evidence'])})")
        print(f"    - Cross Component Evidence:       {tc['cross_component_evidence']} ({', '.join(b['cross_component_evidence'])})")
        print(f"    - Reference Interoperability:     {tc['reference_interoperability']} ({', '.join(b['reference_interoperability'])})")
        print(f"    - Safe Scope Guards:              {tc['safe_scope_guard']} ({', '.join(b['safe_scope_guard'])})")
        print(f"    - Defensive Validation:           {tc['defensive_validation']} ({', '.join(b['defensive_validation'])})")
        print(f"    - Deferred Execution Boundary:    {tc['deferred_execution_boundary']} ({', '.join(b['deferred_execution_boundary'])})")
        print(f"    - Unknown:                        {tc['unknown']}")
        if args.output:
            out_p = Path(args.output)
            out_p.parent.mkdir(parents=True, exist_ok=True)
            out_p.write_text(json.dumps(eff, indent=2), encoding="utf-8")
            print(f"[+] Wrote effective contract to: {out_p}")
    except Exception as e:
        print(f"[FAIL] Error building effective B5F contract: {e}", file=sys.stderr)
        sys.exit(1)


if __name__ == "__main__":
    main()
