...
const patches = [
  ...
  // 15. Phase R5.4: 4-Tier FPS Separation (SRC/RX/DEC/PRES) & Silence decode-pause spam
  {
    name: 'Patch 15: Phase R5.4 4-Tier FPS Separation (SRC/RX/DEC/PRES) & Silence decode-pause spam',
    find: 'let xe={timestamp:0,bytesReceived:0,framesDecoded:0},Qe=0,te=!1;async function je(){if(!A)return null;try{const P=await A.getStats();let ue=0,_e=null;for(const Se of P.values())if(Se.type==="candidate-pair"&&Se.state==="succeeded"){_e=Se,ue=(Se.currentRoundTripTime||0)*1e3;break}if(!_e){for(const Se of P.values())if(Se.type==="candidate-pair"&&(Se.nominated||Se.selected)){_e=Se,Se.currentRoundTripTime!==void 0&&(ue=Se.currentRoundTripTime*1e3);break}}let Ue="UDP p2p";if(_e){const Se=P.get(_e.localCandidateId);if(Se){const Te=(Se.protocol||"udp").toUpperCase();Se.candidateType==="relay"?Ue=`${Te} relay`:Ue=`${Te} p2p`}}for(const Se of P.values())if(Se.type==="inbound-rtp"&&Se.kind==="video"){const Te=Se.timestamp,Ye=xe.timestamp?(Te-xe.timestamp)/1e3:0;let gt=0,Tt=Se.framesDecoded||0,Vt=0,Gt=0;if(s.value&&p)gt=p.currentFps.toFixed(0),Tt=p.totalFramesDecoded,Vt=.5,Gt=0;else{const nt=Tt-xe.framesDecoded;gt=Ye>0?(nt/Ye).toFixed(0):0,Gt=Se.jitterBufferDelay/(Se.jitterBufferEmittedCount||1)*1e3||0,Vt=Se.totalDecodeTime/(Tt||1)*1e3||0,Ye>0&&nt===0&&!te&&t.value==="connected"?(Qe++,te=!0,Bn("[VideoTrace] decode-pause",{pauseCount:Qe,ts:Date.now(),dtMs:Math.round(Ye*1e3),framesDecoded:Tt,bytesReceived:Se.bytesReceived,pliCount:Se.pliCount||0,packetsLost:Se.packetsLost||0,jitterBufferDelay:Se.jitterBufferDelay,jitterBufferEmittedCount:Se.jitterBufferEmittedCount})):nt>0&&(te&&qm("[VideoTrace] decode-resume",{ts:Date.now(),newFrames:nt,framesDecoded:Tt,pliCount:Se.pliCount||0,packetsLost:Se.packetsLost||0}),te=!1)}const ar=Ye>0?((Se.bytesReceived-xe.bytesReceived)*8/Ye/1e3).toFixed(0):0,Ni=Gt.toFixed(0),Pi=(ue+Gt+Vt+10).toFixed(0),Ce=Se.pliCount||0,Ke=Se.packetsLost||0;return xe={timestamp:Te,bytesReceived:Se.bytesReceived,framesDecoded:Tt},{fps:gt,bitrate:ar,jbDelay:Ni,e2eDelay:Pi,rtt:ue.toFixed(0),pliCount:Ce,pauseCount:Qe,lostCount:Ke,connectionType:Ue}}}catch{}return null}function ke(){xe={timestamp:0,bytesReceived:0,framesDecoded:0},Qe=0,te=!1}',
    replace: 'let xe={timestamp:0,bytesReceived:0,framesDecoded:0,framesReceived:0,framesPresented:0},Qe=0,te=!1;async function je(){if(!A)return null;try{const P=await A.getStats();let ue=0,_e=null;for(const Se of P.values())if(Se.type==="candidate-pair"&&Se.state==="succeeded"){_e=Se,ue=(Se.currentRoundTripTime||0)*1e3;break}if(!_e){for(const Se of P.values())if(Se.type==="candidate-pair"&&(Se.nominated||Se.selected)){_e=Se,Se.currentRoundTripTime!==void 0&&(ue=Se.currentRoundTripTime*1e3);break}}let Ue="UDP p2p";if(_e){const Se=P.get(_e.localCandidateId);if(Se){const Te=(Se.protocol||"udp").toUpperCase();Se.candidateType==="relay"?Ue=`${Te} relay`:Ue=`${Te} p2p`}}const _vEl=g?g():null;if(_vEl&&_vEl.requestVideoFrameCallback&&!_vEl.__rvfcHooked){_vEl.__rvfcHooked=!0,_vEl.__presCount=0;function _rvLoop(n_w,m_t){if(m_t&&typeof m_t.presentedFrames=="number"){_vEl.__presCount=m_t.presentedFrames}else{_vEl.__presCount=(_vEl.__presCount||0)+1}if(_vEl&&_vEl.requestVideoFrameCallback){_vEl.requestVideoFrameCallback(_rvLoop)}}_vEl.requestVideoFrameCallback(_rvLoop)}for(const Se of P.values())if(Se.type==="inbound-rtp"&&Se.kind==="video"){const Te=Se.timestamp,Ye=xe.timestamp?(Te-xe.timestamp)/1e3:0;let gt=0,Tt=Se.framesDecoded||0,Vt=0,Gt=0,rxT=Se.framesReceived||0,presT=_vEl?(_vEl.__presCount||Tt):Tt;if(s.value&&p)gt=p.currentFps.toFixed(0),Tt=p.totalFramesDecoded,Vt=.5,Gt=0;else{const nt=Tt-(xe.framesDecoded||0);gt=Ye>0?(nt/Ye).toFixed(0):0,Gt=Se.jitterBufferDelay/(Se.jitterBufferEmittedCount||1)*1e3||0,Vt=Se.totalDecodeTime/(Tt||1)*1e3||0,Ye>0&&nt===0&&!te&&t.value==="connected"?(Qe++,te=!0):nt>0&&(te=!1)}const dRx=rxT-(xe.framesReceived||0),dPres=presT-(xe.framesPresented||0),rxFps=Ye>0?Math.round(dRx/Ye):0,presFps=Ye>0?Math.round(dPres/Ye):0,srcFps=Se.framesPerSecond!==undefined?Math.round(Se.framesPerSecond):rxFps,ar=Ye>0?((Se.bytesReceived-xe.bytesReceived)*8/Ye/1e3).toFixed(0):0,Ni=Gt.toFixed(0),Pi=(ue+Gt+Vt+10).toFixed(0),Ce=Se.pliCount||0,Ke=Se.packetsLost||0,fLbl=`SRC ${srcFps} | RX ${rxFps} | DEC ${gt} | PRES ${presFps}`;window.__streamStats={timestamp:Date.now(),mode:"webrtc",srcFps,rxFps,decodeFps:Number(gt),presentFps:presFps,fpsLabel:fLbl,bitrateKbps:Number(ar),avgJitterBufferMs:Number(Ni),avgDecodeMs:Math.round(Vt*10)/10,rttMs:Number(ue.toFixed(0)),packetLossTotal:Ke,framesReceived:rxT,framesDecoded:Tt,framesPresented:presT};return xe={timestamp:Te,bytesReceived:Se.bytesReceived,framesDecoded:Tt,framesReceived:rxT,framesPresented:presT},{fps:gt,fpsLabel:fLbl,srcFps,rxFps,decodeFps:Number(gt),presentFps:presFps,bitrate:ar,jbDelay:Ni,e2eDelay:Pi,rtt:ue.toFixed(0),pliCount:Ce,pauseCount:Qe,lostCount:Ke,connectionType:Ue}}}catch{}return null}function ke(){xe={timestamp:0,bytesReceived:0,framesDecoded:0,framesReceived:0,framesPresented:0},Qe=0,te=!1}'
  },
